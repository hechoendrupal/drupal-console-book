...
[chain](chain.md) | 链式执行命令
[help](help.md) | Displays help for a command
[init](init.md) | 拷贝配置文件到用户的 home 目录
[list](list.md) | 列出当前可用命令
[server](server.md) | 运行PHP内建的网站服务器
**cache**  |
[cache:rebuild](cache-rebuild.md) | 重建以及清除所有站点缓存
**config**  |
[config:debug](config-debug.md) | 显示当前配置
[config:edit](config-edit.md) | 编辑选中配置
[config:export](config-export.md) | 导出当前应用/网站配置
[config:export:content:type](config-export-content-type.md) | 导出一个指定的内容类型及其字段
[config:export:single](config-export-single.md) | 导出单个配置为 YAML 格式文件
[config:export:view](config-export-view.md) | 导出一个视窗配置到一个模块里的YAML格式文件，这样其他网站也可以使用它.
[config:import](config-import.md) | 导入配置到当前程序.
[config:import:single](config-import-single.md) | 导入所选配置.
[config:override](config-override.md) | 覆盖当前使用中的配置值.
**container**  |
[container:debug](container-debug.md) | 显示程序的当前服务.
**create**  |
...
[cron:execute](cron-execute.md) | 执行模块中的定时任务或全部定时任务
[cron:release](cron-release.md) | 释放定时任务系统锁，以便再次运行定时任务
**database**  |
[database:client](database-client.md) | 启动一个可用数据库客户端
[database:connect](database-connect.md) | 如果有数据库客户端，启动它
[database:dump](database-dump.md) | 导出数据库
[database:log:clear](database-log-clear.md) | 在DBLog表中删除事件日志，可以使用过滤器
[database:log:debug](database-log-debug.md) | 显示程序的当前事件日志
[database:restore](database-restore.md) | 恢复数据库，结构以及数据
...
**generate**  |
[generate:authentication:provider](generate-authentication-provider.md) | 生成一个认证提供者
[generate:command](generate-command.md) | 生成 Console 命令
[generate:controller](generate-controller.md) | 生成并注册一个控制器
[generate:doc:dash](generate-doc-dash.md) | 生成 DrupalConsole.docset 包，提供给 Dash
[generate:doc:gitbook](generate-doc-gitbook.md) | 生成命令文档
[generate:entity:bundle](generate-entity-bundle.md) | 生成内容类型（node 或实体 bundle）
[generate:entity:config](generate-entity-config.md) | 创建新的配置实体
[generate:entity:content](generate-entity-content.md) | 创建新的内容实体
[generate:event:subscriber](generate-event-subscriber.md) | 生成一个事件订阅
[generate:form](generate-form.md) | 生成新的"FormBase"
[generate:form:alter](generate-form-alter.md) | 生成hook_form_alter()或hook_form_FORM_ID_alter的实现程序
[generate:form:config](generate-form-config.md) | 生成新的"ConfigFormBase"
[generate:module](generate-module.md) | 生成一个模块.
[generate:permissions](generate-permissions.md) | 生成模块权限
[generate:plugin:block](generate-plugin-block.md) | 生成一个插件区块
[generate:plugin:condition](generate-plugin-condition.md) | 生成插件条件
[generate:plugin:field](generate-plugin-field.md) | 生成字段类型，Widget 以及格式化器插件
...
[generate:plugin:rest:resource](generate-plugin-rest-resource.md) | 生成插件REST资源
...
[site:install](site-install.md) | 安装Drupal
